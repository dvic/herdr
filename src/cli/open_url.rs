...
fn execute(
    ...
    if let Err(err) = crate::open_url::validate(url) {
        let response = ErrorResponse {
            id: REQUEST_ID.into(),
            error: ErrorBody {
                code: "invalid_params".into(),
                message: format!("invalid URL ({})", err.class()),
            },
        };
        return CommandOutput {
    ...
    let response = match send_request(&request) {
        ...
        Err(err) => {
            return CommandOutput {
                stdout: None,
                stderr: Some(format!("herdr open-url request failed: {err}")),
                exit_code: 1,
            };
        }
    };
...
mod tests {
    use super::*;
    use crate::api::schema::{ClientOpenUrlReason, SuccessResponse};
