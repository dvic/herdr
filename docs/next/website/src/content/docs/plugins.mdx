...
checked in manifest order inside each plugin. A matching handler keeps
precedence; when no handler accepts the link, Herdr opens it on the foreground
full-app client's desktop.
