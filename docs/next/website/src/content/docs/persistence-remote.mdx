...
In this mode your local Herdr is a thin client. It connects over SSH, starts or attaches to the remote Herdr server, and streams the UI back to your local terminal. Because the client runs locally, Herdr can bridge local desktop features into the remote session: image clipboard paste copies the image to a remote temp file and pastes that path, while `herdr open-url` and unhandled modified-click links open in the foreground client's local browser. If no foreground full-app client is attached, URL delivery fails instead of guessing a server desktop.
