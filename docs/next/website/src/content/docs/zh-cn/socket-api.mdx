...
| 客户端 | `client.window_title.set`、`client.window_title.clear`、`client.open_url` |
| 会话 | `session.snapshot` |
...
| 智能体 | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send`、`agent.prompt`、`agent.rename`、`agent.focus`、`agent.start` |
...
在前台完整应用客户端中打开绝对 HTTP(S) URL:

```json
{"id":"req_url","method":"client.open_url","params":{"url":"https://example.com/docs?q=1#install"}}
```

响应是 `type: "client_open_url"`，包含 `delivered` 和 `forwarded`、`dispatched_locally`、`no_foreground_client`、`forward_failed`、`local_dispatch_failed` 之一的 reason。observer 和 controller 连接不会收到该消息。
