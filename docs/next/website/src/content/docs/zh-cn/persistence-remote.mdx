...
这种模式下,你本地的 Herdr 是瘦客户端。它通过 SSH 连接,启动或连接远程 Herdr 服务器,并把 UI 流式传回本地终端。因为客户端在本地运行,Herdr 可以把本地桌面功能桥接到远程会话: 图像剪贴板粘贴会把图像复制到远程临时文件再粘贴该路径，`herdr open-url` 和未处理的修饰键链接则在前台客户端的本地浏览器中打开。没有前台完整应用客户端时，URL 投递会失败，而不会猜测服务器桌面。
