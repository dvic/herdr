...
- open an HTTP(S) URL on the foreground full-app client
...
| Client | `client.window_title.set`, `client.window_title.clear`, `client.open_url` |
...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send`, `agent.prompt`, `agent.rename`, `agent.focus`, `agent.start` |
...
Open an absolute HTTP(S) URL on the foreground full-app client:

```json
{"id":"req_url","method":"client.open_url","params":{"url":"https://example.com/docs?q=1#install"}}
```

The response is `type: "client_open_url"` with `delivered` and reason
`forwarded`, `dispatched_locally`, `no_foreground_client`, `forward_failed`, or
`local_dispatch_failed`. Observer and controller connections are never
