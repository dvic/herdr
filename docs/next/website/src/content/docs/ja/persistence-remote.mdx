...
このモードでは、ローカルの Herdr はシンクライアントです。SSH 越しに接続し、リモートの Herdr サーバーを起動またはアタッチして、UI をローカルターミナルにストリーミングします。クライアントがローカルで動くため、Herdr はローカルデスクトップ機能をリモートセッションにブリッジできます。画像クリップボードの貼り付けは画像をリモートの一時ファイルにコピーしてそのパスを貼り付け、`herdr open-url` と未処理の修飾キー付きリンクはフォアグラウンドクライアントのローカルブラウザで開きます。フォアグラウンドのフルアプリクライアントがなければ、サーバーのデスクトップを推測せず URL 配信は失敗します。
