...
| クライアント | `client.window_title.set`、`client.window_title.clear`、`client.open_url` |
...
| エージェント | `agent.list`、`agent.get`、`agent.read`、`agent.explain`、`agent.send`、`agent.prompt`、`agent.rename`、`agent.focus`、`agent.start` |
...
フォアグラウンドのフルアプリクライアントで絶対 HTTP(S) URL を開きます:

```json
{"id":"req_url","method":"client.open_url","params":{"url":"https://example.com/docs?q=1#install"}}
```

レスポンスは `type: "client_open_url"` で、`delivered` と `forwarded`、`dispatched_locally`、`no_foreground_client`、`forward_failed`、`local_dispatch_failed` のいずれかの reason を含みます。observer と controller 接続は配信対象になりません。
